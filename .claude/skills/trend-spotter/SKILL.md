---
name: trend-spotter
description: エンタメ・ライフスタイルのトレンド情報を隔週で収集してMarkdownにまとめ、アーカイブを管理する
...
- **東京 注目カフェ**（新規オープン・初出店カフェ、話題のカフェ、SNSで人気のカフェなど）
- **東京 グルメ**（今ホットなレストラン、話題の居酒屋、予約困難店、新オープンの飲食店、コスパ最強の隠れた名店など）
- **東京 ファッション**（新規オープン・初出店のファッションショップ、注目ブランド、ポップアップストアなど）
...
収集対象は過去2週間以内の情報とし、直近でオープン・話題になったものを優先する。
...
### Step 2: アーカイブファイルの作成
`docs/archive/YYYY-MM-DD.md` に今回のトレンド情報を保存する（YYYY-MM-DD は収集実行日）。
...
```html
---
title: YYYY-MM-DD エンタメ・ライフスタイルトレンド（隔週）
date: YYYY-MM-DD
---

# YYYY-MM-DD エンタメ・ライフスタイルトレンド
...
## 💬 今回のトレンド考察
この2週間のエンタメ・ライフスタイル全体のトレンドをまとめた考察（流行の方向性、注目ポイントなど）...
```

### Step 3: アーカイブ一覧の取得
`Bash` で既存のアーカイブファイルを一覧取得する：
```bash
ls docs/archive/*.md 2>/dev/null | sort -r | head -26
```
...
> AIが隔週で自動収集するエンタメ・ライフスタイルトレンド｜最終更新: YYYY-MM-DD
...
## 💬 今回のトレンド考察
この2週間のエンタメ・ライフスタイル全体のトレンドをまとめた考察...
